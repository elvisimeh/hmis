...
$w_criteria = "WHERE account_subledgers.subledgername = '{$rad}' AND pay_transaction_tbl.status = 'P' GROUP BY pay_transaction_tbl.visitno,pay_transaction_tbl.prn,
fname,oname,sname";
...
<strong style="color:white;font-size:1.4em">Radiology Waiting List</strong>
</div>
...
<tbody>
    ...
    <tr>
        <td><?= $sn++ ?>.</td>
        <td><button onclick="rad_details('<?=$waiting_list['visitno'] ?>','<?=$waiting_list['pat_cat'] ?>')" style="cursor:pointer"><?= $waiting_list['prn'] ?></button></td>
...
clearInterval(rad_getWaiting_result);
    ...
    rad_getWaiting = setInterval(function(){    
    $.post("rad/sse_waiting_list",{
        id : sse
    },function(data){
...
function jun(){
    $('#result').load('rad/sse_waiting_list',
    function(){
        sse = $('#sse').val();
    });
}
