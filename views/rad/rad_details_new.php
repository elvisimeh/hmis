...
pay_transaction_tbl.prn = patient_data.prn LEFT JOIN users_tbl ON users_tbl.id = pay_transaction_tbl.orderbyid";
$w_criteria = "WHERE pay_transaction_tbl.visitno = '{$vsn}' AND account_subledgers.subledgername = '{$rad}'";
...
$w_criteria = "WHERE paylater_transaction_tbl.visitno = '{$vsn}' AND account_subledgers.subledgername = '{$rad}'";
$test_details = $obj->selects_join($w_fields,$w_tbl,$w_joins,$w_criteria);
}
...
<body>
    ...
    <td><textarea name="comment" id="<?= $test_detail['id'] ?>" class="form-control" cols="10" rows="2"></textarea><span style="display:none;color:red" id="<?= 'error'.$test_detail['id'] ?>">Enter Comment</span></td>
    <td><i class="fa fa-toggle-on" onClick="rad_accept_pat('<?= $vsn ?>','<?= $test_detail['itemid'] ?>','<?= $test_detail['id'] ?>')" style="color:#3FDBBC; transform:scale(1.7);cursor:pointer"></i>
    </td>
    </tr>
    <?php } 
else{ ?>
    ...
    <td><?= $test_detail['tdate'] ?></td>
    <td><?= $test_detail['names'] ?></td>
    <td><input type="text" class="form-control" value="Accepted" disabled></td>
