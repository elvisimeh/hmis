...
<div class="table-responsive" id="result" style="border:0">
    ...
    <td><i class="fa fa-toggle-on" onClick="rad_accept_pat('<?= $vsn ?>','<?= $test_detail['itemid'] ?>','<?= $test_detail['id'] ?>','<?= $cat ?>')" style="color:#3FDBBC; transform:scale(1.7);cursor:pointer"></i>
    </td>
    </tr>
    <?php } 
else{ ?>
...
<?php } } ?>
    ...
    <button onclick="rad_waiting_list()" style="cursor:pointer"><< Waiting List</button>
    </div>
    </div></div>
    </div></div>
    </div>
