...
<strong style="color:white;font-size:1.4em">View Radiology Results</strong>
</div>
...
<div class="col-md-6">
        <select name="" id="sel_pat_rad" class="searchable_inv"></select>
    </div>
...
$('.searchable_inv').select2({
    ...
    ajax: { 
   url: "rad/search_pat_result_rad.php",
   type: "post",
   dataType: 'json',
   delay: 250,
 ...
 $('#sel_pat_rad').change(function(){
    var prn = $('#sel_pat_rad').val();
    rad_result_per_prn(prn);
 });

 function rad_result_per_prn(prn){
    $('#workspace').load('rad/rad_show_all_results?prn='+prn);
 }
