...
$obj = new select;
$w_fields = "lab_inv_waiting_list.visitno,lab_inv_waiting_list.prn,fname,oname,sname,
...
lab_inv_waiting_list.prn = patient_data.prn";
$w_criteria = "WHERE lab_inv_waiting_list.status = 'A' GROUP BY lab_inv_waiting_list.visitno,lab_inv_waiting_list.prn,fname,oname,sname ";
...
$w_criteria_check = "WHERE account_subledgers.subledgername = '{$lab}'";
...
clearInterval(getWaiting_result);
    ...
    getWaiting_result = setInterval(function(){    
    $.post("lab/sse_pending_results",{
        id : sse
    },function(data){
...
function jun(){
    $('#result').load('lab/sse_pending_results',
    function(){
        sse = $('#sse').val();
    });
}
