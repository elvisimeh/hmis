...
<div class="table-responsive" id="result" style="border:0">
    ...
    <button onclick="lab_result_per_prn('<?= $result_details[0]['prn'] ?>')" style="cursor:pointer"><< Back</button>
    <button onclick="generate_pdf('<?= $result_details[0]['id']?>')">pdf</button>
    </div>
    </div></div>
    </div></div>
    </div>
...
var id = "<?= $id ?>";
   window.open('http://localhost/hims/views/lab/report_to_pdf?id='+id,'_blank');
   //window.print();
 /*   var doc = new jsPDF();
