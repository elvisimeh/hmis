...
$w_fields = "lab_inv_waiting_list.prn,all_items_tbl.itemname,users_tbl.names,patient_data.fname,
patient_data.oname,patient_data.sname,lab_sample_type.sample,lab_inv_waiting_list.status,
collection_time,collection_date,lab_inv_waiting_list.inv,lab_inv_waiting_list.id";
$w_tbl = "lab_inv_waiting_list";
$w_joins = "LEFT JOIN all_items_tbl ON lab_inv_waiting_list.inv = all_items_tbl.id LEFT JOIN patient_data ON 
lab_inv_waiting_list.prn = patient_data.prn LEFT JOIN users_tbl ON 
users_tbl.id = lab_inv_waiting_list.collected_by LEFT JOIN lab_sample_type ON 
lab_inv_waiting_list.sample_type = lab_sample_type.id ";
$w_criteria = "WHERE lab_inv_waiting_list.visitno = '{$vsn}' ORDER BY lab_inv_waiting_list.id ASC";
...
<div class="table-responsive" id="result" style="border:0">
    ...
    <td><?= $test_detail['collection_time'] ?></td>
    <td><?= $test_detail['collection_date'] ?></td>
    <td><?= $test_detail['names'] ?></td>
    <td><?= $test_detail['sample'] ?></td>
    <td><i onclick="post_with_template('<?= $test_detail['prn'] ?>','<?= $test_detail['inv'] ?>','<?= $vsn ?>','<?= $test_detail['id'] ?>')" 
...
<?php } } ?>
    ...
    <button onclick="post_lab_results()" style="cursor:pointer"><< Pending Results List</button>
    </div>
    </div></div>
    </div></div>
    </div>
