...
$obj = new select;
$w_fields = "lab_result.id,lab_result.prn,lab_result.posted_date,lab_result.posted_time,all_items_tbl.itemname,oname,sname,fname";
$w_tbl = "lab_result";
$w_joins = "LEFT JOIN patient_data ON lab_result.prn = patient_data.prn LEFT JOIN all_items_tbl ON all_items_tbl.id = lab_result.inv ";
$w_criteria = "WHERE lab_result.prn = '{$prn}' ORDER BY lab_result.id DESC";
...
<tbody>
    ...
    <tr>
        <td><?= $sn++ ?>.</td>
        <td><button onclick="lab_result_details('<?= $result_list['id'] ?>')" style="cursor:pointer"><?= $result_list['prn'] ?></button></td>
        <td><?= $result_list['itemname']; ?></td>        
        <td><?= $result_list['posted_date']; ?></td>
        <td><?= $result_list['posted_time']; ?></td>
    </tr>
    <?php } //} ?>
</tbody>
</table>
</div>
<button onclick="view_lab_results()" style="cursor:pointer"><< Back</button>
</div>
